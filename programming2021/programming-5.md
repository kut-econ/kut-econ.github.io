...
この合計40バイトの配列はメモリ上で一続きになっており、各8バイトの5つの区画に分かれています。それぞれの区画には、リストの要素になっているオブジェクトのアドレスが格納されています。第1回の講義で学んだように、64ビットOSではメモリアドレスを8バイトで表すことに注意しましょう。
...
Pythonのデータ型の中には、それを操作するための専用の関数をもつものがあります。このような関数を**メソッド**と呼びます。(メソッドについては、後の講義で詳しく解説します。)リストのメソッドの中に、copyという自身の複製を作成するメソッドが有ります。これを呼び出すには、次のようにします。
...
これをpythonでは**シャローコピー**(shallow copy)と呼びます。こうしたことは、要素がイミュータブルな場合には全く問題になりません。シャローコピーが問題になるのは、リストがミュータブルな要素を持つときです。

たとえば次の例を考えてみましょう。ここでは、zというリストを作成したうえで、リストxの第2要素にzへの参照を持たせています。さらにxのコピーを作ってyに代入すると、xとyは異なるオブジェクトとなりましたが、当然第2要素のzを共有したままです。従って、x[2](つまりz)の要素を変更すると、yにまで影響が及びます。
...
スライス記法は左辺に現れた場合と右辺に現れた場合でかなり意味合いが異なるので注意しなくてはなりません。
...
具体的な値は、次のように、ソースコードの最初のほうで変数に置き換えてしまうのが望ましいです。ついでに、結果が見やすいように5つごとに改行するようにしましょう。
...
Pythonでは、字下げによってコードブロックを表現するという、他の言語にはない際だった特徴があります。このおかげで、Pythonは非常に見やすいコードを書くことが可能になっているのですが、一方で、うっかりバグを作りやすい性質でもあるので注意が必要です。

括弧記号によってブロックを表現するRのような言語では、コードの編集中に一つのキーを誤って叩いただけで、コードの意味が変貌してしまうというようなことはほとんど起きません。大抵は、エラーを出してコードが止まるだけです。

ところが、Pythonでは、容易にこういったことが起き得るので注意しなくてはなりません。
...
このように、forやifブロックがあると、コーディング中にうっかりタブキーを一つ挿入すると、エラーが出ない上にコードを破壊してしまう可能性があるので、ブロックの字下げには十分に注意しなくてはなりません。特にブロックが入れ子になっている場合はこういった失敗に気づきにくいので要注意です。エディターの自動インデント機能もこういった失敗につながりやすいでしょう。
