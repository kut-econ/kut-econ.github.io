...
- [第5回 データ型と制御構文](#第5回-データ型と制御構文)
  ...
  - [リストの変形と演算](#リストの変形と演算)
  - [ループ処理](#ループ処理)
    - [練習3(ループ処理その1)](#練習3ループ処理その1)
  - [rangeオブジェクト](#rangeオブジェクト)
    - [練習4(ループ処理その2)](#練習4ループ処理その2)
...
上記のコードを実行する際にリアルタイムで何が起きているのかをデバッグモードで確かめることにします。`GitHub/vars`ディレクトリの下に適当なファイル(たとえば"shallow.py")を作成し、次のコードを記述しなさい。
...
`id(x)`、`id(y)`、`x is y`、`x[2] is y[2]`をウォッチ式に設定しなさい。デバッグを実行し、ステップインによってウォッチ式と各変数の値がどのように変化するか観察しなさい。

### 練習2(シャローコピーその2)

次の2つのコードについて、`x`の変更(ともに3行目)は`y`に影響を及ぼすか?結果を予想してからデバッグモードで確かめなさい。ただし、以下のウォッチ式を設定すること。
