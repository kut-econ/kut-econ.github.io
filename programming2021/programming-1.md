...
- [第1回 プログラミング概論](#第1回-プログラミング概論)
  ...
  - [補遺(メモリ上での数の表現)](#補遺メモリ上での数の表現)
    ...
    - [エンディアン](#エンディアン)
  - [注釈](#注釈)
    - [コンパイラとアセンブラのインストール](#コンパイラとアセンブラのインストール)
...
一方、64ビット対応したOSでは、メモリアドレスの表現に8バイト数値を使用します。したがって(少なくとも理論上は)0～2の64乗-1までのアドレスを表現できます。これは極めて巨大な数ですので、64ビットOSではメモリの上限が問題になることは通常ありません。
...
なお、64ビット版のWindows 10にはダンプをするためのツールが標準搭載されていませんが、Gitをインストールすると`xxd`コマンドが使えるようになります。一方Linuxには`xxd`コマンドが標準で搭載されています。いずれのOSでも、ターミナルで次のように入力すれば実行可能ファイルを2進数にダンプできます。
...
Anaconda仮想環境にNASMをインストール済みの人は、アセンブリ言語のソースを書いてアセンブルしてみることもできます(NASMをインストールしていない人は、手順だけ追っておきましょう。)
