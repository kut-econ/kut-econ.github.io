...
プログラムがメモリに読み込まれた（**ロード**されるということもあります）あと、プログラムを構成する命令は、一つずつ命令レジスタと呼ばれるレジスタに格納され、実行されていきます。
...
コンパイルを行うと、a.outという名前のマシン語で書かれたファイルが作成されます。これを**オブジェクトファイル**と呼びます。また、オブジェクトファイルのうち、単体で実行可能なものを**実行可能ファイル**と呼びます。コンパイル型の言語で書かれたソースファイルは直接実行することはできず、かならずコンパイルして実行可能ファイルを作る必要があります。
...
アセンブリ言語にはいくつか方言がありますが、ここではNASM(Netwide Assembler)という表記を用いています。ここでは詳しく説明できませんが、上のコードにおいて、rbpやrsp、eaxは全てレジスタの名前を表します。このように、アセンブリ言語では、レジスタを直接操作することが必要になってきます。
