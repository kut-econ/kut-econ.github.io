...
なお、上記のモデル、疑似データ、回帰直線をプロットすると、以下の図のようになります。こうした図を作成する方法については、次回の講義で取り扱います。
...
時間の都合でここでは扱いませんでしたが、多次元、とくに2次元のNumPy配列はデータ分析上、非常に重要なデータ構造です。参考書やNumPyのリファレンスマニュアルを読んで勉強しておいてください。
