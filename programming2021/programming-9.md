...
ただしこのようにすると、戻されるビューは次元が落ちてしまいます。もとの配列の次元を維持するには、インデックスをリストにする必要があります。
...
これは**ファンシーインデックス**参照と呼ばれる記法の特別な場合です。ファンシーインデックス参照については、次回のpandasデータフレームを説明するところで解説します。
