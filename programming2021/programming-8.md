...
これまでにも、`sys`モジュールや`copy`モジュールを随所で活用してきました。

モジュールは、Pythonスクリプトから読み込んで使用できる関数や変数、データ型、Pythonコードの集合です。

Pythonには大きく分けて次の4種類のモジュールがあります。

|種類|説明|例|
|--|--|--|
|外部ファイル|PythonやCで作成され、importで読み込んで使う|copy|
...
次に、ファイルの形では存在していないビルトインモジュールという特別なタイプのモジュールがあります。たとえば`sys`はビルトインモジュールであり、インタープリタに埋め込まれています。こういったものもimport文で読み込んで使うことができます。
...
このように、`__builtins__`という名前がすでに定義されていて、その正体は`builtins`というビルトインモジュールであることがわかります(ややこしいので混乱しないように注意してください)。これは、`builtins`モジュールが`__builtins__`という別名で読み込まれているということを意味します。モジュールの別名読み込みは次のようにします。
...
最後に、**main**というモジュールがあります(正確には`__main__`)。これは、主スクリプトの属するモジュールです。たとえばコマンドプロンプトで次のようにfile.pyスクリプトを実行したとします。
...
このように、Pythonスクリプトは全てモジュールとして機能しますが、モジュールはより広い概念です。

## モジュールを作る

## 名前空間

名前空間とは、記号と名前を対応づけるものであり、次の3種類があります。

|名前空間|場所|
|--|--|
|ローカル名前空間|関数ブロック|
|グローバル名前空間|モジュールトップレベル|
|ビルトイン名前空間|__builtins__で定義されている名前|
