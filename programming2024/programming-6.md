...
このように、`readlines`メソッドでは文末の改行記号は除去されません。改行を除去したい場合は、次のように書くことができます。
...
`splitlines`は文字列のメソッドであり、文字列を改行で分割し、得られた文のリストを返します(教科書セクション5.2.8参照)。

`readlines`の逆の操作、すなわち文のリストをファイルに書き込むときは、次のように`writelines`メソッドを用います。
...
ただし、この方法では各文の末尾に改行は追加されません。改行を挿入するにはたとえば次のような方法があります。
